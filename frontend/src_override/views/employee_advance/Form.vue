...
function getFilteredFields(fields) {
	...
	const excludeFields = ["naming_series","currency_section","section_break_7"]
	const extraFields = [
		"employee",
		"employee_name",
		"department",
...
function applyFilters(fields) {
	return fields.map((field) => {
		if (["currency", "exchange_rate","advance_account","mode_of_payment","repay_unclaimed_amount_from_salary"].includes(field.fieldname)) {
			field.hidden = true
		}
